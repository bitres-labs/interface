...
async function handlePhishingWarning(page: any, context: any, targetUrl: string): Promise<any> {
  try {
    ...
    if (count > 0) {
      ...
      const warningAgain = await freshPage
        .locator('button:has-text("Continue anyway")')
        .count()
        .catch(() => 0)
      if (warningAgain > 0) {
...
test.describe.serial('Sepolia Integrated E2E Tests', () => {
  ...
  test('1. Mint BTD from WBTC', async ({ page, metamask }) => {
    console.log('\n========== TEST 1: MINT BTD ==========')
    if (page.isClosed()) {
      console.log('[Page closed, completing test]')
      return
    }
  ...
  test('3. Add Liquidity (BTD + USDC)', async ({ page, metamask }) => {
    ...
    if (client) {
      try {
        ...
        reserveBTD =
          token0.toLowerCase() === SEPOLIA_CONTRACTS.BTD.toLowerCase() ? reserves[0] : reserves[1]
    ...
    try {
      ...
      const addBtn = page.locator(
        'button.btn-primary:has-text("Add Liquidity"), button.btn-primary:has-text("Supply")'
      )
        ...
        try {
          ...
          if (actualDeposited > 0n && lpBalance > 0n) {
            ...
            if (ratio >= 0.999 && ratio <= 1.001) {
              ...
              console.log(
                '[Formula] ✗ Farm deposit deviation:',
                ((ratio - 1) * 100).toFixed(2) + '%'
              )
            }
          }
  ...
  test('5. Stake BTD', async ({ page, metamask }) => {
    ...
    try {
      ...
      const stakeBtn = page.locator(
        'button.btn-primary:has-text("Stake BTD"), button.btn-primary:has-text("Stake")'
      )
        ...
        try {
          ...
          if (actualShares > 0n && expectedShares > 0n) {
            ...
            if (ratio >= 0.999 && ratio <= 1.001) {
              ...
              console.log(
                '[Formula] ✗ Stake shares deviation:',
                ((ratio - 1) * 100).toFixed(2) + '%'
              )
            }
          }
  ...
  test('6. Unstake stBTD', async ({ page, metamask }) => {
    ...
    try {
      ...
      const unstakeBtn = page.locator(
        'button.btn-primary:has-text("Unstake"), button.btn-primary:has-text("Withdraw")'
      )
  ...
  test('9. Redeem BTD (CR-dependent outcome)', async ({ page, metamask }) => {
    ...
    try {
      ...
      // Get current CR
      const crText = await page
        .locator('text=/Collateral Ratio|CR/i')
        .textContent()
        .catch(() => '')
      ...
      const btbComp = await page
        .locator('text=/BTB Compensation/i')
        .textContent()
        .catch(() => '')
      const brsComp = await page
        .locator('text=/BRS Compensation/i')
        .textContent()
        .catch(() => '')
      ...
      if (page.isClosed()) {
        console.log('[Redeem BTD] Page closed')
        return
      }
      ...
      if (redeemSuccess && publicClient && account) {
        ...
        try {
          ...
          console.log(
            '[Formula] CR scenario:',
            crPercent >= 100 ? 'Full redemption' : 'Partial + compensation'
          )
  ...
  test('10. Redeem BTB (requires CR >= 100%)', async ({ page, metamask }) => {
    ...
    try {
      ...
      const redeemBtn = page.locator(
        'button.btn-primary:has-text("Convert BTB"), button.btn-primary:has-text("Redeem")'
      )
      if ((await redeemBtn.count()) > 0) {
        const isDisabled = await redeemBtn.first().isDisabled()
        const btnText = await redeemBtn
          .first()
          .textContent()
          .catch(() => '')
